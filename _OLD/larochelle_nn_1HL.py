...
def NN_SGD(GQ_df, binarize, gt, lt, vol, balance_labeled_data, vectorize_label, k, h1, h2, epochs, Lambda, Reg, alpha, plot=False):
  ...
  w2_init = np.sqrt(6)/np.sqrt(outputs+h2)
  W2 = np.random.uniform(low=-w2_init, high=w2_init, size=(outputs*h2)).reshape(outputs,h2)
  ...
  def back_prop(x, W1, b1, W2, b2, z1, a1, z2, a2, f_x, y):
  ...
  def finite_diff_approx(W, b, del_W, del_b, x, f_x, y):
    ...
    for i in xrange(W.shape[0]):
      for j in xrange(W.shape[1]):
        ...
        temp_w = (W[i][j])
        W[i][j] = W[i][j]+epsilon
        z1, a1, z2, a2, f_x = forward_prop(x, W, b1, W2, b2)
        loss_left = loss(f_x,y)[0]
        W[i][j] = temp_w

        W[i][j] = (W[i][j]-epsilon)
        z1, a1, z2, a2, z3, a3, f_x = forward_prop(x, W, b1, W2, b2)
  ...
  for i in xrange(epochs):
    ...
    for x,y in zip(X_train, y_train):
      ...
      z1, a1, z2, a2, f_x = forward_prop(x, W1, b1, W2, b2)
      del_W1, del_b1, del_W2, del_b2 = back_prop(x, W1, b1, W2, b2, z1, a1, z2, a2, f_x, y)
